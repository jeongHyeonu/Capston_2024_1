...
using UnityEngine;
using System.IO;
public class ScreenTexture_VR_image_2 : MonoBehaviour
{
    ...
    */
    ...
    public void Create3()
    {
        ...
        GameObject newMirror = GameObject.Find("Image (" + ImageNum + ")");

        if (onLab == true)
        {
            newMirror=GameObject.Find("Lab_Image (" + ImageNum + ")");
        }

        ImageNum++;

        newMirror.transform.position = MirrorPos.position + new Vector3(105 + 145 * how_x, -85 + how_y * (-145), 0f);
        newMirror.transform.rotation=MirrorPos.rotation;
        newMirror.transform.SetParent(Parent);
        newMirror.transform.localPosition = new Vector3(105 + 145 * how_x, -85 + how_y * (-145), 0f);
        newMirror.transform.localScale = Vector3.one;
        how_x++;
        /*
